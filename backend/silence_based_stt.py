#!/usr/bin/env python3
"""
Silence-based STT using NeMo FastConformer
...
try:
    ...
    print("❌ NeMo not installed. Please install with: pip install nemo_toolkit[asr]")
    sys.exit(1)

# Audio configuration
SAMPLE_RATE = 24000  # Input sample rate
TARGET_SAMPLE_RATE = 16000  # NeMo expects 16kHz
CHUNK_DURATION_MS = 20  # 20ms chunks
CHUNK_SIZE = int(SAMPLE_RATE * CHUNK_DURATION_MS / 1000)  # 1920 samples at 24kHz

# Silence detection parameters
SILENCE_THRESHOLD = 0.01  # RMS threshold for silence detection
SILENCE_TIMEOUT_MS = 200  # 200ms of silence triggers transcription

class SilenceBasedSTT:
    """Silence-based STT processor using NeMo FastConformer"""
    ...
    def __init__(self):
        ...
        self.model = nemo_asr.models.EncDecHybridRNNTCTCBPEModel.from_pretrained(model_name=model_name)
        
        # Set latency configuration
        print("🔧 Setting attention context size to [70, 13] (approx 800ms latency)")
        self.model.encoder.set_default_att_context_size([70, 13])
        
        # Set decoder
        print("🔧 Setting decoder to RNNT")
    ...
    def _warm_up_model(self):
        ...
        if os.path.exists("test_audio.wav"):
            ...
            warmup_result = self.model.transcribe(["test_audio.wav"])
            warmup_time = (time.time() - warmup_start) * 1000
            
            print(f"🔥 Warm-up transcription: {warmup_result[0].text}")
    ...
    def save_audio_to_wav(self, audio_buffer, filename):
        ...
        # Save as WAV file
        sf.write(filename, audio_int16, TARGET_SAMPLE_RATE)
    ...
    def _transcribe_buffer(self):
        ...
        try:
            ...
            if self.keyboard_stop_time is not None and not self.stats_printed:
                ...
                self.save_audio_to_wav(audio_without_silence, str(debug_filename))
            ...
            if result[0].text.strip():
                print(f"\n🎤 Segment {self.segment_count}: '{result[0].text}'")
...
async def stream_microphone_silence_based(device_id=None, show_devices=False):
    ...
    try:
        ...
        if "NeMo not installed" in str(e):
            print("💡 To use silence-based STT, install NeMo with: pip install nemo_toolkit[asr]")
        return
...
async def main():
    parser = argparse.ArgumentParser(description="Silence-based microphone transcription")
    parser.add_argument(
