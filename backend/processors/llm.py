...
class LLMReasoner:
    ...
    def __init__(self):
        ...
        # Load instructions and menu from file
    ...
    def load_instructions_and_menu(self) -> str:
        """Load instructions and menu from menu.txt file"""
        menu_file = Path("menus") / "menu.txt"
        if menu_file.exists():
            try:
                content = menu_file.read_text(encoding='utf-8')
                print(f"📋 Loaded instructions and menu from {menu_file}")
                return content
            except Exception as e:
                print(f"⚠️  Error reading {menu_file}: {e}")
                print("📋 Using default instructions and menu")
        else:
            print(f"⚠️  Menu file not found: {menu_file}")
            print("📋 Using default instructions and menu")
